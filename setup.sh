...
# Wait for boot, SSH into machine, run and clean this file with: sudo bash setup.sh && sudo rm setup.sh
...
cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build && cd build && sudo cp hideaway /usr/bin && sudo chmod +x /usr/bin/hideaway
cd ~
